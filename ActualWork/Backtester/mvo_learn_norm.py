...
def mvo_norm_learn(n_y, n_obs, Q, mu):
    # Variables
    ...
    constraints = [
        cp.sum(phi) == 1, # Sum to 1
        mu.T @ phi >= targetRet, # Target Return Constraint
    ...
    return CvxpyLayer(problem, parameters=[T], variables=[phi])

class mvo_norm_net(nn.Module):
    ...
    def __init__(self, n_x, n_y, n_obs, opt_layer='nominal', prisk='p_var', perf_loss='sharpe_loss',
        ...
        """
        super(mvo_norm_net, self).__init__()
    ...
    def forward(self, X, Y):
        ...
        mu = 1 - (gmean(1+Y.cpu().detach().numpy()))
        ...
        if self.trainT:
            param = self.T
            self.opt_layer = mvo_norm_learn(self.n_y, self.n_obs, Q, mu)
            d = 0
            
        if self.trainDelta:
            param = self.delta
            self.opt_layer = mvo_norm_learn(self.n_y, self.n_obs, Q)
            d = 1
        z_star = self.opt_layer(param, solver_args=solver_args)
