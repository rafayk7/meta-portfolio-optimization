    ...
    def forward(self, X, Y, batching=False):
        ...
        if self.trainDelta:
            ...
            if batching:
                ...
                self.opt_layer = drrpw_nominal_learnDelta_batched(num_assets)
                batched_delta = torch.ones((batch_size,1))*self.delta
                batched_delta = batched_delta.type(torch.FloatTensor)
                z_stars, _ = self.opt_layer(batched_delta, Q)
                ...
                self.opt_layer = drrpw_nominal_learnDelta(Q)
    ...
    def net_train(self, train_set, val_set=None, epochs=None, lr=None, date=None, batching=False):
        ...
        if batching:
            ...
            for epoch in range(epochs):
                ...
                # Store delta for best loss
                if self.loss_val<min_loss:
                    min_loss = loss.item()
                    best_delta = self.delta.item()

                print("Ret: {}. Risk: {}. SR: {}. Loss: {}. Grad: {}. Portfolio: {}. Delta: {}".format(ret, risk, ret/risk, loss.item(), self.delta.grad, {}, self.delta.item()))
                log = False
                if log:
                    writer.add_scalar("Loss/train", loss.item(), epoch)
                    writer.add_scalar("Grad/delta", self.delta.grad, epoch)
                    writer.add_scalar("Value/delta", self.delta.item(), epoch)
                    writer.add_scalar("Value/Return", ret, epoch)
                    writer.add_scalar("Value/Risk", risk, epoch)
                    writer.add_scalar("Value/SharpeRatio", ret/risk, epoch)
                    writer.flush()
                break
        ...
        # self.delta = nn.Parameter(torch.FloatTensor(1).uniform_(best_delta - 0.01,best_delta + 0.01))
